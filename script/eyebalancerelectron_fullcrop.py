...
# Verify the path and list all JPEG files
if not os.path.exists(folder_path):
    ...
    image_files = [f for f in os.listdir(folder_path) if f.endswith(('.jpg', '.jpeg', '.JPG', '.CR3'))]
    ...
    for image_file in image_files:
        ...
        TOP_OF_HEAD_INDEX = 10  # Approximate index for the top of the head
        ...
        if results.multi_face_landmarks:
            ...
            top_of_head = np.array([landmarks[TOP_OF_HEAD_INDEX].x * image_width, landmarks[TOP_OF_HEAD_INDEX].y * image_height]).astype(int)
            ...
            # Extract the new positions for the left eye, right eye, chin, and top of the head
            ...
            new_top_of_head = new_points[TOP_OF_HEAD_INDEX][:2].astype(int)

            # Calculate the headroom and adjust the crop box
            headroom = int(image_height * 0.15)
            y1 = new_top_of_head[1] - headroom
            y1 = max(0, y1)
            distance_to_chin = new_chin[1] - y1
            y2 = y1 + 2 * distance_to_chin
            y2 = min(image_height, y2)

            # Calculate the x-coordinates for cropping based on the 2:3 aspect ratio
            crop_height = y2 - y1
            crop_width = int(crop_height * (2 / 3))
            x_center = (new_left_eye[0] + new_right_eye[0]) // 2
            ...
            output_image_path = os.path.join(output_folder_path, Path(image_file).stem + "_Processed.jpg")
